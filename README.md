...
In addition, NEureka PE grid must be specified with the grid height `PE_H` and grid width `PE_W`; at the moment only the `PE_H=4, PE_W=2` configuration has been thoroughly tested.
Operative modes is controlled with `MODE` parameter: MODE= 0, Perfomance Mode. MODE=1, Resilience Mode.

See the `Makefile` for a full list. All simulation commands should be augmented with modified parameters, e.g.,
```
make stimuli H_IN=7 W_IN=3 K_OUT=32 K_IN=32 PE_H=4 PE_W=2
make sw-all run H_IN=7 W_IN=3 K_OUT=32 K_IN=32 PE_H=4 PE_W=2 gui=0
```
...
- L. Ghionda, R. Tedeschi, Y. Tortorella, A. S. Prasad, D. Rossi, L. Benini and F. Conti "HMR-NEureka: Hybrid Modular Redundancy DNN Acceleration in Heterogeneous RISC-V SoCs," 2025 IEEE Computer Society Annual Symposium on VLSI (ISVLSI), Kalamata, Greece, 2025, pp. 1-6, doi: 10.1109/ISVLSI65124.2025.11130209.
